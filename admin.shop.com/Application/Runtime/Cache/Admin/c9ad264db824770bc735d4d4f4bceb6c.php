        ...
        <div class="col-lg-12">
            <section class="panel">
                ...
                <div style="height: 300px;">
                    <div style="float: left; width: 300px; height: 300px;margin-left: 200px">
                        <p>LOGO:<img src="<?php echo ($row["goods_logo"]); ?>" width="50px"/></p>
                        <p>货号：<?php echo ($row["sn"]); ?></p>
                        <p>宝贝名称：<?php echo ($row["goods_name"]); ?></p>
                        <p>品牌：<?php echo ($row["name"]); ?></p>
                        <p>分类：<?php echo ($row["good_category_name"]); ?></p>
                        <p>库存：<?php echo ($row["stock"]); ?></p>
                        <p>商品描述：<?php echo ($info["goods_content"]); ?></p>

                    </div>
                    <div style="float: right; width: 500px; height: 300px;margin-right: 50px;>
                        <p>LOGO:<img src="<?php echo ($row["goods_logo"]); ?>" width="50px"/></p>
                        <p>市场价格：￥<?php echo ($row["market_price"]); ?></p>
                        <p>本店价格：￥<?php echo ($row["shop_price"]); ?></p>
                        <p>是否上架：<?php if($row["is_on_sale"] == 0 ): ?>下架<?php else: ?> 上架<?php endif; ?></p>
                        <p>显示状态：<?php if($row["status"] == 0 ): ?>关闭<?php else: ?> 显示<?php endif; ?></p>
                        <p>促销活动：
                            <?php if(is_array($rows)): $i = 0; $__LIST__ = $rows;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$c): $mod = ($i % 2 );++$i; echo ($c["info_prom_type"]); ?>,<?php endforeach; endif; else: echo "" ;endif; ?>
                        </p>
                        <p>排序：<?php echo ($row["sort"]); ?></p>
                        <p>录入时间：<?php echo ($row["inputtime"]); ?></p>
                    </div>
                </div>
            </section>
