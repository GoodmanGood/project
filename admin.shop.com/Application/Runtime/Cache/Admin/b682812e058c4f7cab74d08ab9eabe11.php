                        ...
                        <header class="panel-heading">
                           管理员列表
                            <span class="tools pull-right">
                                <a href="<?php echo U('add');?>" class="btn btn-success btn-link">设置管理员</a>
                            </span>
                        </header>
                                ...
                                <table  class="display table table-bordered table-striped" id="dynamic-table">
                                    ...
                                    <tbody>
                                        <?php if(is_array($rows)): $i = 0; $__LIST__ = $rows;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><tr class="gradeX">
                                                ...
                                                <td><?php echo date('Y-m-d,H:m:s',$row['add_time']);?></td>
                                                <td class="center hidden-phone">
                                                    <?php if($row['last_login_ip'] == 0): ?>该管理员尚未登录
                                                        <?php else: ?>
                                                        <?php echo date('Y-m-d,H:m:s',$row['last_login_time']); endif; ?>

                                                </td>
                                                <td class="center hidden-phone">
                                                    <?php if($row['last_login_ip'] == 0): ?>该管理员尚未登录
                                                    <?php else: ?>
                                                    <?php echo long2ip($row['last_login_ip']); endif; ?>
                                                </td>
                                                <td class="center hidden-phone">
                                                    <a href="<?php echo U('edit',['admin_id'=>$row['admin_id']]);?>"  class="btn btn-xs btn-success">编辑</a>|
                                                    <a href="<?php echo U('update',['admin_id'=>$row['admin_id']]);?>"  class="btn btn-xs btn-success">修改密码</a>
                                                |
    ...
    <script type='text/javascript'>
        $(function(){
            $('.deleteBtn').click(function(){
                ...
                var url = "<?php echo U('delete');?>?admin_id="+del_id;
